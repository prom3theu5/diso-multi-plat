...
mesh.export("out/diffmc_sphere_w_deform_cuda.obj")
## Test backward
L = torch.norm(verts)
L.backward()
grad_grid = sdf.grad
...
verts = verts.cpu() * (sphere.aabb[:, 1] - sphere.aabb[:, 0]) + sphere.aabb[:, 0]
mesh = trimesh.Trimesh(vertices=verts.detach().cpu().numpy(), faces=faces.cpu().numpy(), process=False)
mesh.export("out/diffmc_sphere_wo_deform_cuda.obj")
## Test backward
L = torch.norm(verts)
L.backward()
grad_grid = sdf.grad
...
verts = verts.cpu() * (sphere.aabb[:, 1] - sphere.aabb[:, 0]) + sphere.aabb[:, 0]
mesh = trimesh.Trimesh(vertices=verts.detach().cpu().numpy(), faces=faces.cpu().numpy(), process=False)
mesh.export("out/diffdmc_sphere_w_deform_cuda.obj")
## Test backward
L = torch.norm(verts)
L.backward()
grad_grid = sdf.grad
...
verts = verts.cpu() * (sphere.aabb[:, 1] - sphere.aabb[:, 0]) + sphere.aabb[:, 0]
mesh = trimesh.Trimesh(vertices=verts.detach().cpu().numpy(), faces=faces.cpu().numpy(), process=False)
mesh.export("out/diffdmc_sphere_wo_deform_cuda.obj")
## Test backward
L = torch.norm(verts)
L.backward()
grad_grid = sdf.grad
